...
from modules.layer.mast_attention import QueryKeyBlock, CrossAttentionBlock, PoolingQueryKeyBlock

class Hypernetwork(nn.Module):
    def __init__(self, args, input_shape, mixing_hidden_size, last_action = False):
        self.args = args
        ...
        self.agent_embedding = nn.Linear(self.input_shape[0], self.mixing_hidden_size)
        self.enemy_embedding = nn.Linear(self.input_shape[1], self.mixing_hidden_size)
        ...
        if self.args.env_args["state_last_action"] and last_action:
            self.mixing_hidden_size = self.mixing_hidden_size + self.input_shape[2]
            self.add_embed = True
            
        self.cross_attention = CrossAttentionBlock(
            d = self.mixing_hidden_size,
            h = self.n_head,
        )
...
class PIMixer(nn.Module):
    def __init__(self, args, abs = True):
        super(PIMixer, self).__init__()
        
        self.args = args
        self.n_agents = args.n_agents
        self.n_enemies = args.n_enemies
        self.n_allies = self.n_agents - 1
        self.mixing_hidden_size = args.mixing_hidden_size
        ...
        self.hyper_w1 = Hypernetwork(
            ...
            mixing_hidden_size = self.mixing_hidden_size,
        )
        ...
        if self.args.env_args["state_last_action"]:
            self.mixing_hidden_size += state_feature_dims[2]
